...
import { ISvfContent, IFragment, IMaterial, IMesh, IMaterialMap, ILines, IPoints } from '../svf';
...
/**
 * Serializes {@link ISvfContent} into glTF (2.0) format in local folder.
 * @param {ISvfContent} svf Fully parsed SVF content.
 * @param {string} baseDir Path to local folder where the output glTF files should be stored.
 */
export function serialize(svf: ISvfContent, baseDir: string) {
    fse.ensureDirSync(baseDir);
    return new Serializer().serialize(svf, baseDir);
}
...
class Serializer {
    ...
    serialize(svf: ISvfContent, baseDir: string) {
    ...
    protected serializeScene(svf: ISvfContent): gltf.Scene {
    ...
    protected serializeFragment(fragment: IFragment, svf: ISvfContent): gltf.Node {
        let node: gltf.Node = {
    ...
    protected serializeMesh(fragmesh: IMesh, svf: ISvfContent): gltf.Mesh {
        let mesh: gltf.Mesh = {
            primitives: []
        };
    ...
    protected serializeLines(fragmesh: ILines, svf: ISvfContent): gltf.Mesh {
        let mesh: gltf.Mesh = {
            primitives: []
        };
    ...
    protected serializePoints(fragmesh: IPoints, svf: ISvfContent): gltf.Mesh {
        let mesh: gltf.Mesh = {
            primitives: []
        };
    ...
    protected serializeMaterial(mat: IMaterial | null, svf: ISvfContent): gltf.MaterialPbrMetallicRoughness {
        if (!mat) {
    ...
    protected serializeTexture(map: IMaterialMap, svf: ISvfContent): gltf.Texture {
