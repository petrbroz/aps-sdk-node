...
class DataManagementClient {
    ...
    async uploadObjectResumable(bucketKey, objectName, data, byteOffset, totalBytes, sessionId, contentType = 'application/stream') {
        ...
        const response = await common_1.rawFetch(this.host + RootPath + `/buckets/${bucketKey}/objects/${objectName}/resumable`, options);
        if (!response.ok) {
    ...
    async getResumableUploadStatus(bucketKey, objectName, sessionId) {
        ...
        const response = await common_1.rawFetch(this.host + RootPath + `/buckets/${bucketKey}/objects/${objectName}/status/${sessionId}`, options);
        if (response.ok) {
