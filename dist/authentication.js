...
class AuthenticationClient {
     ...
     */
    constructor(client_id, client_secret, host = common_1.DefaultHost) {
    ...
    authenticate(scopes, force = false) {
        ...
        const cache = this._cached[key] = {
            ...
            promise: common_1.post(`${this.host}${RootPath}/authenticate`, { urlencoded: params }).then((resp) => {
    ...
    async getToken(code, redirectUri) {
        ...
        const token = await common_1.post(`${this.host}${RootPath}/gettoken`, { urlencoded: params });
        return token;
    }
}
